...
@section('content')
    <div class="m-portlet m-portlet--tab">
        ...
        <!--begin::Form-->
        <form class="theme-form" action="{{route('article.saveEdit',['id'=>$article->id])}}" method="post" enctype="multipart/form-data" novalidate>
            @csrf
            <div class="m-portlet__body">
                <div class="form-group m-form__group">
                    <label class="col-form-label pt-0" for="exampleInputEmail1">Title</label>
                    <input class="form-control" id="exampleInputEmail1" name="title" type="text" value="{{$article->title}}">
                    @error('title')
                ...
                <div class="form-group m-form__group">
                    ...
                    <input class="form-control" id="exampleInputPassword1" name="description" type="text" placeholder="" value="{{$article->description}}">
                    @error('description')
                    <div id="emailHelp" class="form-text text-danger">{{$message}}</div>
                    @enderror
                </div>
                <div class="form-group m-form__group">
                    ...
                    <div class="col-md-12">
                        <textarea name="content" id="editor" rows="15">{{$article->content}}</textarea><br>
                        @error('content')
                        <div id="emailHelp" class="form-text text-danger">{{$message}}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group m-form__group">
                    ...
                    <div class="col-md-12">
                        <input class="form-control" id="exampleInputPassword1" name="url" type="text" placeholder="" value="{{$article->url}}">
                        @error('url')
                        <div id="emailHelp" class="form-text text-danger">{{$message}}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group m-form__group">
                    <label class="col-form-label pt-0" for="exampleInputPassword1">Chuyên mục</label>
                    <div class="col-md-12">
                        <select class="form-select form-control" aria-label="Default select example" name="cate_id">
                            @foreach($categories as $cate)
                                <option value="{{$cate->id}}" @if($article->cate_id == $cate->id) selected @endif>{{$cate->name}}</option>
                            @endforeach
                        </select>
                        @error('cate_id')
                            <div id="emailHelp" class="form-text text-danger">{{$message}}</div>
                ...
                <div class="form-group m-form__group">
                    ...
                    <div class="col-md-12">
                        <select class="form-select form-control" aria-label="Default select example" name="single_page">
                            <option @if ($article->single_page == 1)  selected @endif value="1">trang đơn</option>
                            <option @if ($article->single_page == 0)  selected @endif value="0">bài viết</option>
                        </select>
                    </div>
                </div>
            </div>
